...
async function updateRumRiverExperienceIcons() {
  try {
    ...
    // Update the block with new icon values (using icon names instead of emojis)
    const updatedBody = [...homeStory.content.body];
    const experienceBlock = updatedBody[experienceIndex];

    experienceBlock.features = [
      {
        ...experienceBlock.features[0],
        icon: 'sparkles'
      },
      {
        ...experienceBlock.features[1],
        icon: 'star'
      },
      {
        ...experienceBlock.features[2],
        icon: 'heart'
      },
      {
        ...experienceBlock.features[3],
        icon: 'check'
      }
    ];

    console.log('Updating story with new icon values...');
    ...
    console.log('✅ Rum River Experience icons successfully updated!');

  } catch (error) {
    console.error('❌ Error updating Rum River Experience icons:', error.message);
    if (error.response) {
